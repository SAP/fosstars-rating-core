...
1.  **[Security testing](#security-testing)**: **7.08** (weight is 1.0)
...
More info:
1.  [List of issues on LGTM](https://lgtm.com/projects/g/psf/requests)
...
Score: **7.08**, confidence is 10.0 (max), weight is 1.0 (high)
...
1.  **Number of projects on GitHub that use an open source project:** 1086025
1.  **Number of stars for a GitHub repository:** 46213
1.  **Number of watchers for a GitHub repository:** 1378
...
1.  [CVE-2014-1829](https://nvd.nist.gov/vuln/detail/CVE-2014-1829): Requests (aka python-requests) before 2.3.0 allows remote servers to obtain a netrc password by reading the Authorization header in a redirected request.
1.  [CVE-2015-2296](https://nvd.nist.gov/vuln/detail/CVE-2015-2296): The resolve_redirects function in sessions.py in requests 2.1.0 through 2.5.3 allows remote attackers to conduct session fixation attacks via a cookie without a host value in a redirect.
1.  [CVE-2017-16026](https://nvd.nist.gov/vuln/detail/CVE-2017-16026): Request is an http client. If a request is made using ```multipart```, and the body type is a ```number```, then the specified number of non-zero memory is passed in the body. This affects Request >=2.2.6 <2.47.0 || >2.51.0 <=2.67.0.
...
1.  [CVE-2021-29476](https://nvd.nist.gov/vuln/detail/CVE-2021-29476): Requests is a HTTP library written in PHP. Requests mishandles deserialization in FilteredIterator. The issue has been patched and users of `Requests` 1.6.0, 1.6.1 and 1.7.0 should update to version 1.8.0.
1.  [CVE-2021-21674](https://nvd.nist.gov/vuln/detail/CVE-2021-21674): A missing permission check in Jenkins requests-plugin Plugin 2.2.6 and earlier allows attackers with Overall/Read permission to view the list of pending requests.
